...
public class FXMLhostController implements Initializable {
    ...
    private void addstay(ActionEvent event) {
        ...
        if(errors.length()>0){
            ...
            ss.add(new Stay(Integer.parseInt(tfcapacity.getText()),
                AuthentificationController.idglobal, tfdescription.getText(),
    ...
    private void updatestay(ActionEvent event) {
        ...
        TrayNotification tray = new TrayNotification();
            ...
            tray.setMessage("You successuffully updated a stay in our application");
            tray.setNotificationType(NotificationType.SUCCESS);
            tray.showAndDismiss(Duration.millis(2000));
            refreshlist();
        }
    ...
    public void refreshlist(){
        data.clear();
        data=FXCollections.observableArrayList(ss.findbyIdHosT(u.getId()));
